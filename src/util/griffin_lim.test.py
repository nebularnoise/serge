...
plt.imshow(np.transpose(spectrum_mag), origin='lower',
           extent=(0, length_seconds * 2000, 0, n_fft/2 * 4))
plt.show()
...
plt.figure()
plt.imshow(np.absolute(np.transpose(s_reconstruct)), origin='lower',
           extent=(0, length_seconds * 2000, 0, n_fft/2 * 4))
plt.show()
