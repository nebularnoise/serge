...
%which had to be shut down due to a lack of time.
...
.3 README.txt \DTcomment{Martin Fouilleul}.
.3 build \DTcomment{Martin Fouilleul}.
.3 *.py \DTcomment{Martin Fouilleul}.
.3 src.
.4 *.cpp \DTcomment{Martin Fouilleul}.
.2 src.
.3 train.
.4 *.py \DTcomment{Antoine Caillon / Thibault Geoffroy}.
.3 util.
.4 MCNN.py \DTcomment{Adrien Bitton}.
.4 audio utilities.py \DTcomment{github/bkvogel}.
.4 *.py \DTcomment{Martin Fouilleul}.
.3 *.py, *.ipynb \DTcomment{Antoine Caillon}.
}
\vspace{1cm}
The \textbf{faust} directory contains the .dsp code describing the effects used to augment the second dataset. The \textbf{notebooks} directory is where the first AE - VAE - WAE experiments has been made. Inside \textbf{pd-external} is the source code for compiling the \textbf{PureData} external \textbf{vae\_sampler}, alongside some test patches. The \textbf{src} directory contains \textbf{final\_model\_log\_scale.py} which is the Python script that has been extensively used to train a WAE on spectrograms from our plucked strings dataset\footnote{Note that this dataset is available under \texttt{/u/atiam.1819/caillon/dev/serge/dataset/string.tar.gz}.}. The \textbf{explore\_space.py} loads a pretrained model and make you listen to it interactively.\newline
